...
class Nepal(CountryVaxBase):
    ...
    def _parse_pdf_table(self) -> pd.Series:
        ...
        # Checks data
        check_known_columns(df, ['Unnamed: 0', 'Covid Shield', 'Unnamed: 1', 'Verocell', 'Unnamed: 2', 'J & J', 'Pfizer', 'Unnamed: 3', 'Moderna'])
    ...
    def _parse_metrics(self, ds: pd.Series) -> tuple:
        ...
        assert len(count_list) == 13, "New column is added in the table, please update the script. Number of columns: {}".format(len(count_list))
    ...
    def pipe_manufacturer_vaccine(self, df:pd.DataFrame)->pd.DataFrame:
        ...
        return df.assign(**{"vaccine" : ["Oxford/AstraZeneca", "Sinopharm/Wuhan", "Johnson&Johnson", "Pfizer/BioNTech", "Moderna"]})
    ...
    def pipeline_manufacturer(self, df:pd.DataFrame)->pd.DataFrame:
        ...
        return df.pipe(self.pipe_manufacturer_date).pipe(self.pipe_manufacturer_vaccine).pipe(self.pipe_manufacturer_location)
    ...
    def export(self):
        ...
        #Export to CSV
        self.export_datafile(df=df_main,df_manufacturer=df_manufacturer,attach=True,meta_manufacturer={
